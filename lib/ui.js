...
function setupUI(config){
  ...
  screen.append(buildTitleBox([{
    ...
    label: 'yarn',
    value: child_process.execSync('yarn --version')
  }].concat(info || [])));
  ...
  childrenConfig.push({
    file: '../client/testServer.js',
    args: karma ? [`--config=${karmaConfig}`, `--module=${karma}`] : [],
    logBoxConfig: { top: 0, right: 0, height: '50%', label: 'Tests'}
  });

  var children = childrenConfig.reduce((children, config) => {
